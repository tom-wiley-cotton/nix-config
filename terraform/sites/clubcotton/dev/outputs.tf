...
locals {
  ...
  instances = {
    ...
    "homeassistant_macvlan" = {
      name     = incus_instance.homeassistant_macvlan.name
      type     = incus_instance.homeassistant_macvlan.type
      status   = incus_instance.homeassistant_macvlan.status
      location = incus_instance.homeassistant_macvlan.target
      ip       = incus_instance.homeassistant_macvlan.ipv4_address
      profile  = module.profiles.haos_macvlan_profile_name
      resources = {
        cpu    = "2"
        memory = "4GB"
        disk   = "64GB"
    ...
    "dev_container" = {
      name     = incus_instance.dev_container.name
      type     = incus_instance.dev_container.type
      status   = incus_instance.dev_container.status
      location = incus_instance.dev_container.target
      ip       = incus_instance.dev_container.ipv4_address
      profile  = module.profiles.container_profile_name
      resources = {
